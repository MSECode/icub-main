...
{
    bool PinholeCalibrationEngine::calibrate(
        ...
        std::string& errorMessage) const
    {
        ...
        try
        {
            if(!options.isValid())
            {
                errorMessage = "Invalid pinhole calibration options.";
                return false;
            }
        ...
        {
            errorMessage = std::string("OpenCV exception during calibration: ") + e.what();
        }
        catch(const std::exception& e)
        {
            errorMessage = std::string("Standard exception during calibration: ") + e.what();
        }
        catch(...)
        {
            errorMessage = "Unknown exception during calibration.";
        }
    ...
    bool PinholeCalibrationEngine::validateObservations(
        const std::vector<StereoObservation>& observations, 
    ...
    bool PinholeCalibrationEngine::calibrateMonocular(
        const std::vector<StereoObservation>& observations, 
        ...
        std::string& errorMessage) const
    {
        ...
        if(options.cameraFocalLengthGuess > 0.0 && (options.monocularFlags & cv::CALIB_USE_INTRINSIC_GUESS))
        {
    ...
    bool PinholeCalibrationEngine::calibrateStereo(
        const std::vector<StereoObservation>& observations,
        const PinholeCalibrationOptions& options, 
    ...
    bool PinholeCalibrationEngine::extractCalibrationPoints(
        const std::vector<StereoObservation>& observations, 
        std::vector<std::vector<cv::Point3f>>& objectPoints,
        std::vector<std::vector<cv::Point2f>>& leftImagePoints, 
        std::vector<std::vector<cv::Point2f>>& rightImagePoints,
